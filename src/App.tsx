...
import { categoryTotals, detailTotals, formatINR, monthTransactions, spendingTotal, totalFor } from './analytics'
import { askAi, providerDefaults, testAiConnection, type AiConfig, type AiProvider, type FinancialAiContext } from './ai'
import { addTransaction, deleteTransaction, loadCategories, loadTransactions } from './storage'
...
function App() {
  ...
  const spending = spendingTotal(currentMonth)
  ...
  function saveTransaction(amount: number, type: TransactionType, categoryId: string, details: string) {
    const transaction: Transaction = {
      id: crypto.randomUUID(), amount, type, categoryId, details: details.trim() || undefined,
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === 'analytics' && <Analytics transactions={transactions} categories={categories} onDelete={(id) => { deleteTransaction(id); setTransactions(loadTransactions()); setNotice('Transaction deleted'); window.setTimeout(() => setNotice(''), 2600) }} />}
...
function Analytics({ transactions, categories, onDelete }: { transactions: Transaction[]; categories: Category[]; onDelete: (id: string) => void }) {
  ...
  const details = detailTotals(month, categories)
  const detailMax = Math.max(...details.map((entry) => Math.abs(entry.amount)), 1)
  return <div className="page"><section className="page-heading"><p className="eyebrow">Financial overview</p><h1>Analytics</h1><p>Simple signals from your real transaction history.</p></section><div className="analytics-summary"><Metric label="Balance" value={totalFor(transactions, 'income') - totalFor(transactions, 'expense')} tone="blue" /><Metric label="Total income" value={totalFor(month, 'income')} tone="green" /><Metric label="Spending (excl. investments)" value={spendingTotal(month)} tone="coral" /></div><section className="chart-panel"><div className="section-heading"><div><p className="eyebrow">Distribution</p><h2>Monthly spending</h2></div><span className="period-pill">Current month</span></div>{month.length ? <div className="bars">{totals.map((item) => <div className="bar-item" key={item.id}><div className="bar-track"><div className={`bar-fill ${item.id}`} style={{ height: `${Math.max(item.amount ? 10 : 2, item.amount / max * 100)}%` }} /></div><span>{item.icon}</span><small>{item.name}</small><strong>{formatINR(item.amount)}</strong></div>)}</div> : <div className="empty-chart"><LineChart size={24} /><span>Your charts will appear after you add transactions.</span></div>}</section>{details.length > 0 && <section className="chart-panel detail-chart"><div className="section-heading"><div><p className="eyebrow">Your entries</p><h2>By source or purpose</h2></div></div><div className="detail-bars">{details.slice(0, 6).map((item) => <div className="detail-bar" key={item.name}><span>{item.name}</span><div className="detail-track"><div className={`detail-fill ${item.amount >= 0 ? 'positive' : ''}`} style={{ width: `${Math.max(8, Math.abs(item.amount) / detailMax * 100)}%` }} /></div><strong>{formatINR(item.amount)}</strong></div>)}</div></section>}<section className="history-panel"><div className="section-heading"><div><p className="eyebrow">Activity log</p><h2>Registered transactions</h2></div><span className="period-pill">{month.length} this month</span></div>{transactions.length ? <div className="transaction-list">{transactions.slice(0, 12).map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} categories={categories} onDelete={onDelete} />)}</div> : <p className="history-empty">Your registered transactions will appear here.</p>}</section></div>
}

function TransactionRow({ transaction, categories, onDelete }: { transaction: Transaction; categories: Category[]; onDelete: (id: string) => void }) {
  const category = categories.find((item) => item.id === transaction.categoryId)
  const isIncome = transaction.type === 'income'
  return <div className="transaction-row"><span className={`transaction-icon ${isIncome ? 'income' : 'expense'}`}>{category?.icon ?? '✦'}</span><span className="transaction-info"><strong>{transaction.details || category?.name || 'Other'}</strong><small>{category?.name ?? 'Other'} · {new Date(`${transaction.date}T00:00:00`).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</small></span><strong className={isIncome ? 'amount-positive' : 'amount-negative'}>{isIncome ? '+' : '-'}{formatINR(transaction.amount)}</strong><button className="delete-button" onClick={() => onDelete(transaction.id)} aria-label={`Delete ${transaction.details || category?.name || 'transaction'}`}><X size={15} /></button></div>
}
...
function EntrySheet({ categories, onClose, onSave }: { categories: Category[]; onClose: () => void; onSave: (amount: number, type: TransactionType, categoryId: string, details: string) => void }) { const [amount, setAmount] = useState(''); const [type, setType] = useState<TransactionType>('expense'); const [categoryId, setCategoryId] = useState(categories[0]?.id ?? 'other'); const [details, setDetails] = useState(''); const [error, setError] = useState(''); function submit() { const value = Number(amount); if (!amount || !Number.isFinite(value) || value <= 0) { setError('Please enter a valid amount.'); return } if (!details.trim()) { setError(type === 'income' ? 'Please add where this money came from.' : 'Please add what this money was for.'); return } onSave(value, type, categoryId, details) } return <div className="scrim" onClick={onClose}><div className="entry-sheet" onClick={(event) => event.stopPropagation()}><div className="sheet-top"><div><p className="eyebrow">New transaction</p><h2>Capture the moment</h2></div><button className="close-button" onClick={onClose} aria-label="Close"><X size={18} /></button></div><label className="amount-field"><span>₹</span><input autoFocus inputMode="decimal" value={amount} onChange={(event) => { setAmount(event.target.value); setError('') }} placeholder="0" aria-label="Amount" /></label>{error && <p className="form-error">{error}</p>}<div className="segmented"><button className={type === 'expense' ? 'selected' : ''} onClick={() => { setType('expense'); setError('') }}>Spend</button><button className={type === 'income' ? 'selected' : ''} onClick={() => { setType('income'); setError('') }}>Income</button></div><label className="details-field"><span>{type === 'income' ? 'Where did this money come from?' : 'What was this money for?'}</span><input value={details} onChange={(event) => { setDetails(event.target.value); setError('') }} placeholder={type === 'income' ? 'Salary, freelance, refund...' : 'Lunch, rent, travel...'} /></label><div className="entry-categories">{categories.map((category) => <button key={category.id} className={categoryId === category.id ? 'selected' : ''} onClick={() => setCategoryId(category.id)}><span>{category.icon}</span>{category.name}</button>)}</div><button className="primary-button" onClick={submit}>Add transaction <ArrowUpRight size={17} /></button></div></div> }
